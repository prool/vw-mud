...
/*sprintf( buf, "%d   ", dnew->host_ip);
   write_to_buffer( dnew,  buf, 0, 0);
*/    return;
}
#endif
    ...
    if ( fPrompt && !merc_down && d->connected == CON_PLAYING )
    {
	   ...
	   if ( IS_SET(ch->act, PLR_PROMPT) )
	   {
		  ...
		  else if (ch->position == POS_FIGHTING)
		  {
			 ...
			 {
				ADD_COLOUR(ch, cond, L_RED, MAX_INPUT_LENGTH);
			 }
			 else if ((victim->hit*100/victim->max_hit) < 50)
			 {
				ADD_COLOUR(ch, cond, L_BLUE, MAX_INPUT_LENGTH);
			 }
			 else if ((victim->hit*100/victim->max_hit) < 75)
			 {
				ADD_COLOUR(ch, cond, L_GREEN, MAX_INPUT_LENGTH);
			 }
			 ...
			 {
				ADD_COLOUR(ch, cond, L_CYAN, MAX_INPUT_LENGTH);
			 }
...
/* the anti-trigger version, which randomly inserts extra spaces into the output stream */
...
void nanny( DESCRIPTOR_DATA *d, char *argument )
{
    ...
    case CON_CONFIRM_NEW_NAME:
	   ...
	   case CON_GET_NEW_SEX:
		  ...
		  snprintf(stat, MAX_STRING_LENGTH, "Str %d, ",ch->pcdata->perm_str); strncat( buf, stat, MAX_STRING_LENGTH-strlen(buf) );
		  snprintf(stat, MAX_STRING_LENGTH, "Int %d, ",ch->pcdata->perm_int); strncat( buf, stat, MAX_STRING_LENGTH-strlen(buf) );
		  snprintf(stat, MAX_STRING_LENGTH, "Wis %d, ",ch->pcdata->perm_wis); strncat( buf, stat, MAX_STRING_LENGTH-strlen(buf) );
		  snprintf(stat, MAX_STRING_LENGTH, "Dex %d, ",ch->pcdata->perm_dex); strncat( buf, stat, MAX_STRING_LENGTH-strlen(buf) );
		  snprintf(stat, MAX_STRING_LENGTH, "Con %d." ,ch->pcdata->perm_con); strncat( buf, stat, MAX_STRING_LENGTH-strlen(buf) );
		  strncat( buf, "]: \n\rTo keep these stats type 'keep'.  Anything else rerolls." , MAX_STRING_LENGTH-strlen(buf));
		  write_to_buffer( d, buf, 0, 0 );
    ...
    for ( ; to != NULL; to = to->next_in_room )
    {
	   ...
	   while ( *str != '\0' )
	   {
		  ...
		  else
		  {
			 ...
			 case 'o':
				if (obj1 != NULL) snprintf(kav, MAX_INPUT_LENGTH, "%s's",obj1->short_descr);
				i = can_see_obj( to, obj1 )
				    ? ( (obj1->chobj != NULL && obj1->chobj == to)
				    ? "your" : kav)
				    : "something's";
